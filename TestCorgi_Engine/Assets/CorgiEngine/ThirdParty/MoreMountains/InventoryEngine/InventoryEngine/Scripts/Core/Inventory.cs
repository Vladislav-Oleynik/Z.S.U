...
namespace MoreMountains.InventoryEngine
{
	...
	public class Inventory : MonoBehaviour, MMEventListener<MMInventoryEvent>, MMEventListener<MMGameEvent>
	{
		...
		public enum InventoryTypes { Main, Equipment, Store }
		...
		{
			if (InventoryType == Inventory.InventoryTypes.Main || InventoryType == Inventory.InventoryTypes.Store)
			{
		...
		/// <param name="index">Index.</param>
		/// <param name="slot">Slot.</param>
		public virtual void BuyItem(InventoryItem item, int index, InventorySlot slot = null)
		{
            //TODO: implement item purchase mechanics

            if (item.Buyable) 
			{
				Debug.LogError("Buyable");
				Debug.LogError(GetQuantity("RetroCoin"));
				if (GetQuantity("RetroCoin") >= 1)
                {
					Debug.LogError("RetroCoin >= 1");
		...
		public virtual void OnMMEvent(MMInventoryEvent inventoryEvent)
		{
			...
			switch (inventoryEvent.InventoryEventType)
			{
				...
				case MMInventoryEventType.BuyRequest:
					BuyItem(inventoryEvent.EventItem, inventoryEvent.Index, inventoryEvent.Slot);
					break;
			}
		}

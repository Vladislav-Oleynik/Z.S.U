...
{	
	...
	public class InventorySlot : Button
	{
		...
		{
			//now we dont buy, we just get chosen item
			//TODO: change method Buy()
			if (!SlotEnabled) { return; }
			MMInventoryEvent.Trigger(MMInventoryEventType.BuyRequest, this, ParentInventoryDisplay.TargetInventoryName, ParentInventoryDisplay.TargetInventory.Content[Index], 0, Index, ParentInventoryDisplay.PlayerID);
		}
		...
		/// <summary>
		/// Returns true if the item at this slot can be bought, false otherwise
		/// </summary>
		public virtual bool Buyable()
		{
			//TODO: change method Buyable()
			if (InventoryItem.IsNull(ParentInventoryDisplay.TargetInventory.Content[Index]))
			{
				return false;
			}
			if (!ParentInventoryDisplay.TargetInventory.Content[Index].IsBuyable)
			{
				return false;
			}
			else
		...
		public virtual bool BuyButtonShouldShow()
		{
			if (InventoryItem.IsNull(ParentInventoryDisplay.TargetInventory.Content[Index])) { return false; }
			return ParentInventoryDisplay.TargetInventory.Content[Index].DisplayProperties.DisplayBuyButton;
		}
